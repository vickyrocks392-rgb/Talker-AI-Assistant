# Noryx Security Architecture

This document describes the security hardening for **Noryx**. It covers the threat model, the centralised security services, and the protections across input, output, files, RAG, API, and secrets.
...
All security logic is centralised under `server/security/` and exposed through a single barrel (`server/security/index.ts`). No security logic is scattered across routes — every endpoint delegates to a reusable, testable service.

| Service | File | Responsibility |
|---------|------|----------------|
| `InputSecurityService` | `InputSecurityService.ts` | Sanitize, normalize, validate user input. |
...
**Configurable limits** (env overrides): `SEC_MAX_INPUT_LENGTH`, `SEC_MAX_NEWLINES`, `SEC_MAX_REPEATED_CHARS`, `SEC_MAX_TOKEN_REPETITIONS`.
...
The AI Monitor includes a **Security** section. For every request, `SecurityTelemetry` aggregates:
...
**No sensitive user data is ever displayed.** Only counters, category labels, and dispositions are stored. Frontend: `src/components/AIMonitorPanel.tsx` renders the Security card.
...
```mermaid
flowchart LR
    U[Untrusted User Input] -->|sanitize + filter| R[Route]
    R -->|prompt injection scan| Rag[RAG Query/Document]
    R -->|validate + scan| F[File Upload]
    R -->|output filter| LLM[LLM Output]
    LLM -->|filtered reply| U
    Sec[Secrets / Config] -.->|audited, masked| Logs[(Logs)]
```

- **Boundary 1 (Input):** all user text is sanitized and filtered before reaching business logic.
- **Boundary 2 (RAG):** queries and document text are scanned for injection; retrieval is scoped to attached documents only.
- **Boundary 3 (Output):** LLM output is filtered before leaving the server.
- **Boundary 4 (Secrets):** secrets are audited at startup and never logged or returned.
...
2. **Heuristic content filtering** — rule-based; may produce false positives/negatives. Allowlist tuning recommended.
